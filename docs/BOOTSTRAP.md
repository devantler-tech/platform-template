...
Bootstrap validates that the heartbeat URL uses HTTPS port 443 and derives its
DNS hostname into the same encrypted cluster Secret for the Coroot network
policy; no separate hostname secret or input is required.
